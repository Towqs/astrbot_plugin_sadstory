...
@register("astrbot_plugin_sadstory", "Towqs", "伤感故事插件 - 以合并转发形式在群聊中展示伤感故事", "0.3.2")
class SadStoryPlugin(Star):
    ...
    async def initialize(self):
        self._reload_config()
        logger.info(f"[SadStory] 插件初始化完成，主讲人: {len(self.protagonist_qq_list)}个, 网友: {len(self.bystander_qq_list)}个")
    ...
    def _reload_config(self):
        ...
        logger.info(f"[SadStory] 配置加载: 主讲人QQ={self.protagonist_qq_list}, 网友QQ={self.bystander_qq_list}, 素材群={self.source_group_id}")
        # 合并用户池（此时只有从群拉取的成员，QQ号对应的昵称在使用时动态获取）
    ...
    def _resolve_qq_lists(self, all_members: list):
        """根据 protagonist_qq_list 和 bystander_qq_list 从群成员中匹配昵称"""
        member_map = {u["user_id"]: u for u in all_members}

        self.custom_protagonists = []
        for qq in self.protagonist_qq_list:
            if qq in member_map:
                self.custom_protagonists.append(member_map[qq])
            else:
                self.custom_protagonists.append({"nickname": f"用户{qq[-4:]}", "user_id": qq})

        self.custom_bystanders = []
        for qq in self.bystander_qq_list:
            if qq in member_map:
                self.custom_bystanders.append(member_map[qq])
            else:
                self.custom_bystanders.append({"nickname": f"用户{qq[-4:]}", "user_id": qq})
